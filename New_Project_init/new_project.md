...
- [ ] Name your top-level package directory `{project_id}/` (not `src/`) — e.g., `aethervault/` for AetherVault. This prevents namespace collisions with sibling projects
...
- [ ] Move `docs/` and `assets/` into `src/` so non-editable pip installs work
...
- [ ] Implement CLI argument parsing in `src/main.py` with `--version`, `--debug`, `--upgrade`
